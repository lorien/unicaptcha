...
async_uni.html. In each document there must be usage examples for 2captcha and for
my project. Two columns in each document: in the left column are 2captcha code
...
- Generate all possible examples: every challenge type my library supports, plus
  account balance and reporting a bad result. If a 2captcha example is about a
  challenge type which is not supported by my library yet, do not show such
  example. Skip 2captcha's manual submit/poll flow too.
...
- Use plain monospace styling and Python syntax highlighting with an appropriate
  light theme: white or very light background for code blocks, not a dark one.
