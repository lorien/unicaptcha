...
**Status:** Accepted (amends ADR-0051's parity principle; complements ADR-0007; `api_key` union per ADR-0063; `referral` kwarg per ADR-0072; amended 2026-08-24: facade constructors take `api_key` positional and everything else keyword-only — `*` after `api_key`, mirroring the Solver rule)
