...
**Status:** Accepted (amends the adapter SDK sketch in architecture.md; complements ADR-0050 and ADR-0056; `ParsedTask` typed surface formalized by ADR-0075; renamed 2026-08-24: `parse_task_result` → `parse_task_status`, enum `TaskState` → `TaskStatus`)
...
`parse_task_status(raw) -> ParsedTask # pending|ready|unsolvable`:
...
- **`parse_task_status` returns four states**: `pending | ready |
