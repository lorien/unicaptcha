...
Runnable scripts for the [unicaptcha](https://github.com/lorien/unicaptcha)
library, one per use case, in blocking (`sync/`) and asyncio-native
(`async/`) flavors.
...
Per-kind extras for each provider are documented in the challenge-class
docstrings and in `spec/docs/architecture.md` §2 of the repository.
...
|---|---|
| `universal_client.py` | `Solver`/`AsyncSolver` multi-adapter dispatch + capability introspection |
